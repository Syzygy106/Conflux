...
abstract contract PoolOwnable {
  ...
  function _setPoolOwner(PoolKey memory key, address owner) internal {
    PoolId id = key.toId();
    _owners[id] = owner;
  ...
  function poolOwner(PoolKey memory key) public view returns (address) {
    return _owners[key.toId()];
  }

  /// @notice Reverts if caller is not the owner of the pool
  modifier onlyPoolOwner(PoolKey memory key) {
    require(msg.sender == poolOwner(key), "PoolOwnable: caller is not the pool owner");
    _;
  }

  /// @notice Transfer pool ownership to a new address
  function transferPoolOwnership(PoolKey memory key, address newOwner) external onlyPoolOwner(key) {
  ...
  function renouncePoolOwnership(PoolKey memory key) external onlyPoolOwner(key) {
    PoolId id = key.toId();
    address prev = _owners[id];
